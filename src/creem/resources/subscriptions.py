...
class AsyncSubscriptions(AsyncAPIResource):
    ...
    async def pause(self, subscription_id: str) -> Subscription:
        """Temporarily pause a subscription; billing stops until resumed."""
        return await self._client.request(
            "POST", f"/v1/subscriptions/{subscription_id}/pause"
        )

    async def resume(self, subscription_id: str) -> Subscription:
        """Resume a paused (or scheduled-for-cancellation) subscription."""
        return await self._client.request(
            "POST", f"/v1/subscriptions/{subscription_id}/resume"
        )
    ...
    @overload
    async def upgrade(self, subscription_id: str, **kwargs: Any) -> Subscription: ...
